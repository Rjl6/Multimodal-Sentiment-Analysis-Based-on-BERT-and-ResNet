...
parser.add_argument('--fuse_model_type', default='CMAC', help='融合模型类别', type=str)
...
parser.add_argument('--do_test',default=True, action='store_true', help='预测测试集数据')
...
elif config.fuse_model_type == 'bert':
    # config.only = 'text'
...
def train():
    ...
    for e in range(epoch):
        ...
        vloss, vacc, recall_score = trainer.valid(val_loader)
        print('Valid Loss: {}'.format(vloss))
        print('Valid Acc: {}'.format(vacc))
        print('Valid Recall: {}'.format(recall_score))
        if vacc > best_acc:
            best_acc = vacc
