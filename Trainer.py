...
class Trainer():
    ...
    def __init__(self, config, processor, model, device=torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')):
        ...
        elif config.only == 'img':
             resnet_params = set(self.model.full_resnet.parameters())
             other_params = list(set(self.model.parameters()) - resnet_params)
             no_decay = ['bias', 'LayerNorm.weight']
             params = [
                {'params': [p for n, p in self.model.full_resnet.named_parameters() if not any(nd in n for nd in no_decay)],
                    'lr': self.config.resnet_learning_rate, 'weight_decay': self.config.weight_decay},
                {'params': [p for n, p in self.model.full_resnet.named_parameters() if any(nd in n for nd in no_decay)],
    ...
    def valid(self, val_loader):
        ...
        metrics,recall_score = self.processor.metric(true_labels, pred_labels)
        return val_loss / len(val_loader), metrics, recall_score
